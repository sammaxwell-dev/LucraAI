...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const filteredGroups = searchQuery.trim()
    ? {
        today: groupedSessions.today.filter((s) =>
          s.title.toLowerCase().includes(searchQuery.toLowerCase()),
        ),
        yesterday: groupedSessions.yesterday.filter((s) =>
          s.title.toLowerCase().includes(searchQuery.toLowerCase()),
        ),
        thisWeek: groupedSessions.thisWeek.filter((s) =>
          s.title.toLowerCase().includes(searchQuery.toLowerCase()),
        ),
        older: groupedSessions.older.filter((s) =>
          s.title.toLowerCase().includes(searchQuery.toLowerCase()),
        ),
      }
    : groupedSessions;
      ...
      <div className={`${sidebarClasses} bg-[#050505] flex flex-col`}>
        ...
        <div className="px-6 mb-6">
          <div className="relative group w-full">
            <Search
              className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 group-hover:text-zinc-400"
              size={16}
            />
            <input
              type="text"
